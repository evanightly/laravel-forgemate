...
export class WebviewProvider {
     ...
     */
    public async generateBackendFiles(
     ...
     */
    public async generateFrontendFiles(
        model: ModelDefinition,
        projectRoot: string,
        generatedFiles: string[],
    ): Promise<void> {
     ...
     */
    private async generateBackendFilesInternal(
        model: ModelDefinition,
        projectRoot: string,
        generatedFiles: string[],
    ): Promise<void> {
        try {
            ...
            if (model.options?.generateRepository !== false) {
                ...
                await this.templateEngine.generateFile(
            ...
            if (model.options?.generateRepository !== false) {
                const repoPath = path.join(
                    projectRoot,
                    `app/Repositories/${model.name}Repository.php`,
                );
            ...
            if (model.options?.generateController !== false) {
                const controllerPath = path.join(
                    projectRoot,
                    `app/Http/Controllers/${model.name}Controller.php`,
                );
            ...
            if (model.options?.generateApiController !== false) {
                ...
                const apiControllerPath = path.join(
                    apiControllerDir,
                    `${model.name}Controller.php`,
                );
                await this.templateEngine.generateFile(
            ...
            if (model.options?.generateResource !== false) {
                const resourcePath = path.join(
                    projectRoot,
                    `app/Http/Resources/${model.name}Resource.php`,
                );
            ...
            if (model.options?.generateFactory !== false) {
                const factoryPath = path.join(
                    projectRoot,
                    `database/factories/${model.name}Factory.php`,
                );
            ...
            if (model.options?.generateSeeder !== false) {
                const seederPath = path.join(
                    projectRoot,
                    `database/seeders/${model.name}Seeder.php`,
                );
     ...
     */
    private async generateFrontendFilesInternal(
        model: ModelDefinition,
        projectRoot: string,
        generatedFiles: string[],
    ): Promise<void> {
        ...
        try {
            ...
            const modelInterfacePath = path.join(
                jsRoot,
                `Support/Interfaces/Models/${model.name}.ts`,
            );
            ...
            const resourceInterfacePath = path.join(
                jsRoot,
                `Support/Interfaces/Resources/${model.name}Resource.ts`,
            );
            await this.templateEngine.generateFile(
                'frontend/resource',
                model,
            ...
            // Create service hook
            const serviceHookPath = path.join(
                jsRoot,
                `Services/${this.toCamelCase(model.name)}ServiceHook.ts`,
            );
...
`;
    ...
    private toCamelCase(str: string): string {
        return (
            ...
            str.replace(/[\s_-](\w)/g, (_, c) => c.toUpperCase()).substring(1)
        );
    }
     ...
     */
    private async handleScaffoldGeneration(
        model: ModelDefinition,
        ...
        try {
            ...
            vscode.window
                ...
                .then((selection) => {
                    if (selection === 'Show Files') {
                        // Open model file
                        const modelFile = generatedFiles.find((f) =>
                            f.includes(`/Models/${model.name}.php`),
                        );
                        if (modelFile) {
            ...
            vscode.window.showErrorMessage(
                `Error generating scaffold: ${(error as Error).message}`,
            );
        }
    }
    ...
    private async ensureBaselineFilesExist(projectRoot: string): Promise<void> {
        ...
        const baseRepoInterfacePath = path.join(
            projectRoot,
            'app/Support/Interfaces/Repositories/BaseRepositoryInterface.php',
        );
        const serviceHooksFactoryPath = path.join(
            projectRoot,
            'resources/js/Services/serviceHooksFactory.ts',
        );

        if (!fs.existsSync(baseRepoInterfacePath) || !fs.existsSync(serviceHooksFactoryPath)) {
            // If required files are missing, show an error message with initialization instructions
            throw new Error(
                'Required baseline files are missing. Please run "Laravel Forgemate: Initialize Project" command first to create all necessary files.',
            );
        }
    }
    ...
    private validateLaravelProject(projectRoot: string): void {
        ...
        if (
            ...
            throw new Error(
                `The directory at "${projectRoot}" does not appear to be a valid Laravel project`,
            );
        }
    }
